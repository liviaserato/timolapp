    ...
    <>
      <DashboardCard icon={Package} title={t("orders.title")} headerRight={visibilityToggle}>
        ...
        <div className="mt-2 flex flex-col gap-2">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            {mode === "30d" && (
              <span className="text-sm text-foreground text-center w-full sm:w-auto block sm:inline">{periodLabel}</span>
            )}
          </div>
      ...
      <Dialog open={showAllProducts} onOpenChange={(open) => { setShowAllProducts(open); if (open) setProductTab("agua"); }}>
        <DialogContent className="max-w-sm">
          ...
          <div className="h-[170px] overflow-y-auto pr-3 space-y-1.5">
            {(() => {
  ...
  return (
    <div className="rounded-lg border border-app-card-border p-3 flex flex-col items-center text-center">
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span className="text-[13px] sm:text-sm text-muted-foreground leading-tight">{label}</span>
      </div>
