...
export function BinaryTreeLayout({ root, treeRoot, onSelect }: Props) {
  ...
  if (!lc) {
    connectors.push([L_X, CHILD_Y + EMPTY_LABEL_BOTTOM + 4, L_X, CHILD_Y + CHILD_H + EXIT_OFFSET]);
  }
  if (!rc) {
    connectors.push([R_X, CHILD_Y + EMPTY_LABEL_BOTTOM + 4, R_X, CHILD_Y + CHILD_H + EXIT_OFFSET]);
  }
